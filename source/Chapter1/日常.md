...
- [`DropIt`](http://www.dropitproject.com) `W` 中文本地化文件转为 utf-8 格式消除乱码

### 远程桌面

收费：向日葵、TeamViewer、AnyDesk、ToDesk、MobaXterm.

- [`RustDesk`](https://rustdesk.com/) [`gh`](https://github.com/rustdesk/rustdesk) `WLMA`
- [`Remmina`](https://remmina.org/) [`gh`](https://github.com/FreeRDP/Remmina) `L`
- [`mRemoteNG`](https://mremoteng.org/) [`gh`](https://github.com/mRemoteNG/mRemoteNG) `WLM`
- [`PRemoteM`](https://github.com/1Remote/PRemoteM) `W` 与 mRemoteNG 共享配置文件。
...
- [`Files`](https://github.com/files-community/Files/) `W`
- [`Q-Dir`](http://www.q-dir.com/) `W` 四窗口。
- [`Multi Commander`](http://multicommander.com/) `W` 类 TC
- [`FreeCommander`](https://freecommander.com/) `W$`类 TC
...
- [`File Converter`](https://file-converter.org/) `WM` 可以转换音频、视频、图像、文档。

### 文件同步

- [`Syncthing`](https://syncthing.net/) `WLMA` 开源持续文件同步。
- [`FreeFileSync`](https://freefilesync.org/) `WLM` 文件夹比较和同步。
...
- [`VeraCrypt`](https://www.veracrypt.fr/) [`gh`](https://github.com/veracrypt/VeraCrypt) `WLM` 磁盘加密，加密文件、目录、整个磁盘或分区。
- [`Cryptomator`](https://cryptomator.org/) [`gh`](https://github.com/cryptomator/cryptomator) `WLMA` 云同步网盘加密，其中保险库类似 VeraCrypt 容器。
...
- [`age`](https://age-encryption.org/) [`gh`](https://github.com/FiloSottile/age) `WLM` 命令行文件加密工具。
...
- [`SimpleX`](https://simplex.chat/) [`gh`](https://github.com/simplex-chat/simplex-chat) `WLM` 私密通讯。
...
- [`WinDirStat`](https://windirstat.net/contact.html) [`gh`](https://github.com/windirstat/windirstat) `W` 磁盘分析和清理。
- [`Dism++`](https://github.com/Chuyu-Team/Dism-Multi-language) `W` 磁盘清理，系统优化。
- [`Optimizer`](https://github.com/hellzerg/optimizer) `W` 系统优化。
...
- [`Rufus`](https://rufus.ie/) [`gh`](https://github.com/pbatard/rufus) `W`
- [`Ventoy`](https://www.ventoy.net/) [`gh`](https://github.com/ventoy/Ventoy) `WL`

### 桌面整理

- [`CLaunch`](https://hp.vector.co.jp/authors/VA018351/en/index.html) `W`
- [`coodesker`](https://www.coodesker.com/) `W`

### 动态壁纸

- [`Rainmeter`](https://www.rainmeter.net/) `W` 系统美化，自找资源。
- [`Lively Wallpaper`](https://github.com/rocksdanister/lively) `W`
...
- [`Listary`](https://www.listary.com/) `W$` 免费版够用。
- [`Cerebro`](https://github.com/cerebroapp/cerebro) `WLM`
- [`uTools`](http://www.u.tools/) `WLM` 工具丰富，专业版收费。
- [`Wox`](https://github.com/Wox-launcher/Wox) `W` `wox.one` 下载插件。微软 `PowerToys Run` 工具的核心。
- [`Flow Launcher`](https://github.com/Flow-Launcher/Flow.Launcher) `W` 以 Wox 为基础开发的版本。

### 手势工具

通过鼠标描绘图案实现快捷操作。

- [`GestureSign`](https://github.com/TransposonY/GestureSign) `W`
- [`MouseGestureL`](https://hp.vector.co.jp/authors/VA018351/en/mglahk.html) `W`

### 微软出品

- [`PowerToys`](https://github.com/microsoft/PowerToys) `W` 多种实用工具箱。
- [`Sysinternals`](https://learn.microsoft.com/zh-cn/sysinternals/downloads) `W`
...
- [`Koodo Reader`](https://koodoreader.com/) [`gh`](https://github.com/troyeguo/koodo-reader) `WLM` 支持 EPUB、PDF、djvu、MOBI、AZW3
...
- [`ImageMagick`](https://imagemagick.org/) `WL` 图片处理，支持批量，命令行
...
- [`Audacity`](https://www.audacityteam.org/) `W` 跨平台。
...
- [`Musictheory`](https://www.musictheory.net/) 网址
- [`iwasdoingallright`](http://www.iwasdoingallright.com/) 网址

## 视频

### 视频编辑

- [`FFmpeg`](https://ffmpeg.org/) `WLM` 音视频、图片剪辑
- [`Kdenlive`](https://kdenlive.org/) `WLM`
- [`Shotcut`](https://github.com/mltframework/shotcut) `WLM`
- [`OpenShot`](http://www.openshot.org/) [`gh`](https://github.com/OpenShot/openshot-qt) `WLM`
- [`HandBrake`](https://github.com/HandBrake/HandBrake) `WLM`
- [`Olive`](https://olivevideoeditor.org/) [`gh`](https://github.com/olive-editor/olive) `WLM` 非线性视频编辑
- [`Hybrid`](https://www.selur.de/) `WLM` 视频编辑，补帧等
